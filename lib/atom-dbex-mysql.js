...
export default class DbexMySQL {
  ...
  resolveDoubleClick(connectionName, datasets, onDone) {
    ...
    } else if (datasets.schema) {
      command = (connection)=>table.getTables(connection, datasets.schema, onDone, this.logger);
  ...
  resolveActionClick(action, connectionName, datasets, onDone) {
    ...
    if (action === 'structure') {
      ...
      command = (connection)=> database.getSchemas(
        connection,
        (schemas)=>{
          createTable.show(schemas, datasets.schema);
          onDone();
        },
        this.logger
      );
  ...
  refreshNode(connectionName, datasets, onDone) {
    ...
    if (datasets.table) {
      ...
      command = (connection)=>table.getTables(
        connection,
        datasets.schema,
        (tables)=>onDone(table.getTablesNode(datasets.schema, tables)),
        this.logger,
      );
    } else if (datasets.view) {
