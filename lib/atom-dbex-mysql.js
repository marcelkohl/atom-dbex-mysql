...
`;
  ...
  resolveActionClick(action, connectionName, datasets, onDone) {
    ...
    if (action === 'structure' && (datasets.table || datasets.view)) {
      command = (connection)=>this._getDataStructure(
        (datasets.table || datasets.view),
        connection,
        datasets.table ? RT_TABLE : RT_VIEW,
        onDone
      );
    }
  ...
  _getDataStructure(structureName, connection, requestType, onDone) {
    let query = `SHOW CREATE ${requestType} ${structureName}`;
